...
<!--    <MiltyCiteModal :active="activeMiltyCity" @update:active="activeMiltyCity = $event" />-->
    <n-form>
      <n-grid cols="12" x-gap="10" y-gap="15" class="" responsive="screen">
        <n-grid-item span="12" class="mb-3">
          <n-tabs type="segment" animated>
            <n-tab-pane name="One Way" tab="One Way"></n-tab-pane>
            <n-tab-pane name="Round Trip" tab="Round Trip"></n-tab-pane>
            <n-tab-pane name="Multi City" tab="Multi City"></n-tab-pane>
          </n-tabs>
        </n-grid-item>
        <n-grid-item span="5" >
        ...
        <n-grid-item class="bg-white hidden md:block" span="2" id="from-container">
          <div class="flex items-end justify-center h-full text-nive-light text-xl pb-2">
            <Icon class="flex-center" name="heroicons:arrows-right-left-20-solid" />
          </div>
        </n-grid-item>
        <n-grid-item span="5" id="from-container">
          <Airport v-model="payload.to" label="To" :inital-value="payload.to" />
        </n-grid-item>
        <n-grid-item :span="payload.search_type === '0' ? '5':'2'" v-if="payload.search_type !== '1' && payload.search_type !== '2'" id="from-container">
          <n-form-item class="flex-col" :show-feedback="false" label="Depurator">
            <div>
              <n-date-picker
                  v-model:formatted-value="payload.oneWayDate"
                  type="date"
                  id="flight-date"
                  :default-value="payload.roundTripDate"
        ...
        <n-grid-item span="5" v-if="payload.search_type !== '0'" id="from-container">
          <n-form-item class="flex-col"  :show-feedback="false" label="Depurator">
            <div>
              <n-date-picker
                  v-model:formatted-value="payload.roundTripDate"
                  type="date"
                  id="flight-date"
                  :default-value="payload.roundTripDate"
                  :is-date-disabled="disablePreviousDate"
        ...
        <n-grid-item span="5" v-if="payload.search_type !== '0'" id="from-container">
          <n-form-item  class="flex-col"  :show-feedback="false" label="Arrival">
            <div>
              <n-date-picker
                  v-model:formatted-value="payload.roundTripDate"
                  id="flight-date"
        ...
        <n-grid-item span="6" id="from-container">
          <n-form-item class="" :show-feedback="false" label="Cabin">
            <n-select :show-arrow="false" v-model:value="payload.cabin" default-value ='Economy' :options="CabinTypes" />
          </n-form-item>
        </n-grid-item>
        <n-grid-item span="6" id="from-container">
          <n-form-item class="" :show-feedback="false" label="Airline">
        ...
        <n-grid-item span="4" id="from-container" v-for="passenger in PassengersTypes">
          <n-form-item class="" :show-feedback="false" label="Adult">
        ...
        <n-grid-item span="12"  id="from-container">
          <button @click="handelSearch" class="bg-gray-light w-full  flex items-center gap-3 py-3 mt-3 rounded-full relative  justify-center duration-300">
...
<script setup lang="ts">
  ...
  const payload = ref( route.query.q ? {
    ...
    oneWayDate:JSON.parse(route.query.q).legs[0].date || new Date().toISOString().slice(0, 10),
  } : {
        from:null,
        to:null,
        roundTripDate:[
        ...
        oneWayDate:new Date().toISOString().slice(0, 10),
        "search_type": "1",
